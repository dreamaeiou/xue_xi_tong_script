...
(async function (vue, pinia$1, ElementPlus, md5, $$1) {
    ...
    const userConfig = [{
        ...
        config: [{
            ...
            label: "智普清言API密钥",
            type: "input",
            value: defaultConfig$1.zhipu_apiKey,
            desc: "AI密钥"
        }
            /* , {
                name: "thtoken",
    ...
    const _sfc_main$1 = vue.defineComponent({
        components: {}, setup() {
            ...
            { forminput, dialogV, activeName } = pinia$1.storeToRefs(formstoreObj), 
            ruleFormRef = vue.ref(), rules = vue.reactive({
                interval: [{ required: true, message: "间隔时间不能为空" }, 
                    { type: "number", message: "间隔时间必须为数字" }, 
                    { validator: (rule, value) => value >= 1 ? Promise.resolve() : Promise.reject("间隔时间必须大于等于1") }], 
                answerInterval: [{ required: true, message: "答题间隔不能为空" }, 
                    { type: "number", message: "答题间隔必须为数字" }, 
                    { validator: (rule, value) => value >= 1 ? Promise.resolve() : Promise.reject("答题间隔必须大于等于1") }], 
                zhipu_apiKey: [{
                    validator: (rule, value) => {
                        if (value) {
                            return /^[a-zA-Z0-9]{6,}$/.test(value) ? Promise.resolve() : Promise.reject("apikey格式错误");
                        }
            ...
            return {
                dialogV, activeName, ruleFormRef, forminput, rules,
                submitForm: async (formEl) => {
                    alert("submitForm 触发"); // 辅助判断函数是否执行
                    ...
                    formEl.validate((valid, fields) => {
                        ...
                        console.log("即将保存的数据:", forminput.value);
                
                        if (valid) {
                            formstoreObj.saveConfig(forminput.value);
                            console.log("已调用 saveConfig");
    ...
    class ServerApi {
        ...
        async callAIModel(questionData) {
            return new Promise((resolve) => {
                // var apiKey = defaultConfig$1.zhipu_apiKey;
                ...
                // 构建结构化 Prompt
                let instruction = `你是一个专注于答题的智能助手。根据题目类型（type），你需要按照以下规则作答：
                - 若 type = 0：请从题目提供的选项中选择一个正确答案进行回复，必须严格使用选项中的完整内容，不要添加额外解释或说明，也不要添加选项前缀字母（如 A. B.），响应结果为全文字，不需要出现数字，字段间用逗号分隔。
                - 若 type = 1：请从题目提供的选项中选择多个正确答案进行回复，必须严格使用选项中的完整内容，不要添加额外解释或说明，也不要添加选项前缀字母（如 A. B.），响应结果为全文字，不需要出现数字，字段间用逗号分隔。
                - 若 type = 3：请判断题干是否成立，仅回答“是”或“否”。
                请严格按照上述规则作答。\n\n`;

                // 拼接题目内容
                let questionPrompt = `题干：${questionData.question}\n\n选项：\n`;
                if (questionData.options && questionData.options.length > 0) {
                    questionData.options.forEach((opt, index) => {
                        questionPrompt += `${index + 1}. ${opt}\n`;
                    });
                }

                const fullPrompt = instruction + questionPrompt;
                ...
                fetch(url, {
                    ...
                    .then(data => {
                        ...
                        const parsedAnswer = content
                            .split(',')                     // 按逗号切割成数组
                            .map(item => item.trim())     // 去除每项前后的空格
                            .filter(item => item);        // 过滤掉空项（可选）

                        resolve({ form: "AI", answer: parsedAnswer });
                    })
                    .catch(err => {
    ...
    const Typr$1 = getDefaultExportFromCjs(Typr), questionType = { "单选题": "0", "多选题": "1", "填空题": "2", "判断题": "3", "简答题": "4", "名词解释": "5", "论述题": "6", "计算题": "7" }, log = (data, type = "info") => {
        ...
        getAnswers = (questionData, windowz = _unsafeWindow) => {
            ...
            // const promises = [];
            const promises = [server.getAnswer2(questionData), server.getAnswer4(questionData), server.callAIModel(questionData)];
        ...
        _sfc_main = vue.defineComponent({
            setup() {
                ...
                { dialogVisible, count, questionList, task } = pinia$1.storeToRefs(askstore), 
                askActiveName = vue.ref("first"), 
                askActiveNames = vue.ref(["1"]), 
                msg = vue.ref("<h3>本脚本仅用于学习交流</h3><br><p style='color:red;'>禁止用于各种非法用途，否则后果自负</p><br>"), 
                ...
                return {
                    ...
                    submitForm: async (formEl) => {
                        formEl && await formEl.validate((valid, fields) => {
                            valid && (formstoreObj.saveConfig(forminput.value), 
                            console.log('当前表单内容:', forminput.value),
            ...
            await new Promise((resolve) => {
                ...
                const intervalId = setInterval(() => {
                    if ("isUnFinishJob" in iframeWindow && iframeWindow.isUnFinishJob()) {
                        if (player.paused() && playerButton) {
                            playerButton.click(); // 点击播放按钮继续播放
                        }
                    } else {
